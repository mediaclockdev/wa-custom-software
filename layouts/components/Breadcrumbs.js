  ...
  return (
    <nav aria-label="Breadcrumb" className="py-4">
      <div className="container">
        <ol className="inline-flex items-center space-x-1 md:space-x-2 text-sm font-medium bg-white/60 backdrop-blur-md px-5 py-2.5 rounded-full border border-gray-100 shadow-[0_4px_20px_rgb(0,0,0,0.02)]">
          <li className="inline-flex items-center">
            <Link
              href="/"
              className="inline-flex items-center text-gray-500 hover:text-primary transition-colors duration-300"
            >
              <MdHome className="text-lg mr-1.5 text-gray-400" />
              Home
            </Link>
          </li>
          ...
          {pathParts.map((part, index) => {
            ...
            if (part === "page" || (!isNaN(part) && pathParts[index - 1] === "page")) {
              return null;
            }
