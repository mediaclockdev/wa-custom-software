...
const ButtonLink = ({ href = "/", onClick, title = "", animate }) => {
  return (
    <Link
      href={href}
      onClick={onClick}
      className={`relative overflow-hidden px-8 py-3 rounded-full bg-secondary text-white text-lg font-medium transition-all duration-300 group ${animate ? "animate-cta-attention" : ""}`}
    >
      {title}
      <IoIosCall className="inline-block ml-2" size={25} />
      {/* Shine layer */}
